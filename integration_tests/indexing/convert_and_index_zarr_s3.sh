...
# Example commands to do the following with all data on s3:
...
S3_INDEXER=$BASEDIR/s3_indexer/index_from_s3_bucket.py

# Clean s3 prefix to store zarr
S3_BUCKET=easi-dc-data
S3_ZARR_PRE=staging/dev-col540/zarr/index_test
aws s3 rm "s3://$S3_BUCKET/$S3_ZARR_PRE" --recursive

# Geotiff test data
S3_DATA_PRE=staging/dev-col540/odc_test_data/
...
python $S3_INDEXER $S3_BUCKET -p $S3_DATA_PRE --suffix="agdc-metadata.yaml"

# Convert GeoTiff dataset to zarr
$DATACUBE_DRIVER_DIR/utils/convert.py \
    --outdir "s3://$S3_BUCKET/$S3_ZARR_PRE" \
    --chunks x:500 --chunks y:500 \
    "s3://$S3_BUCKET/$S3_DATA_PRE"

# Prepare zarr dataset metadata
$DATACUBE_DRIVER_DIR/examples/prepare_zarr_ls5.py "s3://$S3_BUCKET/$S3_ZARR_PRE/$DATASET_NAME"

# Add to datacube
datacube product add $DATACUBE_DRIVER_DIR/examples/ls5_scenes_zarr.yaml
python $S3_INDEXER $S3_BUCKET -p $S3_ZARR_PRE/$DATASET_NAME --suffix="agdc-metadata.yaml"
