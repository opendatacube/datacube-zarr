...
def test_save_dataarray(protocol, chunks, data, tmpdir, s3):
    ...
    group_name = 'dataset1'
    name = 'array1'
    _save_dataarray(data, protocol, root, group_name, name, chunks['input'])
    ...
    # Load and check data
    ds = _load_dataset(protocol, root, group_name)
...
def test_save_dataset(protocol, chunks, data, tmpdir, s3):
    ...
    group_name = 'dataset1'
    name = 'array1'
    _save_dataset(data, protocol, root, group_name, name, chunks['input'])
...
def test_save_dataarrays(protocol, chunks, data, tmpdir, s3):
    ...
    for set_no in range(1, 3):
        ...
        datasets[set_no] = ds
        _save_dataarray(ds, protocol, root, group_name, name, chunks['input'])
...
def test_save_datasets(protocol, chunks, data, tmpdir, s3):
    ...
    for set_no in range(1, 3):
        ...
        ds = data.copy() * set_no  # Make each dataset a bit different for testing
        datasets[set_no] = ds
        _save_dataset(ds, protocol, root, group_name, name, chunks['input'])
...
def test_overwrite_dataset(protocol, fixed_chunks, data, tmpdir, s3):
    ...
    ds = zio.load_dataset(
        root=str(root), group_name=group_name
    )
...
def test_save_datasets_nested(protocol, data, tmpdir, s3):
    ...
    for i in range(len(groups)):
        ...
        datasets[name] = ds
        _save_dataset(ds, protocol, root, groups[i], name)

    # Load and check data
    for i, (name, dataset) in enumerate(datasets.items()):
        ds = _load_dataset(protocol, root, groups[i])
        assert np.array_equal(dataset, ds[name].values)
...
def test_save_datasets_nested_zarr(protocol, data, tmpdir, s3):
    ...
    for i, r in enumerate(roots):
        ...
        ds = data.copy() * (i + 1)  # Make each dataset a bit different for testing
        datasets[name] = ds
        _save_dataset(ds, protocol, r, "", name)
