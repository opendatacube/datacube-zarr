...
def estimate_compression_ratio(
    ...
    def _slice_range(step: int, stop: int, max_steps=10) -> List[slice]:
    ...
    def _eval_compression_ratio(window: Tuple[slice, ...]) -> float:
        ...
        r = block.nbytes / len(compressor.encode(block.tobytes()))
        return r
    ...
    r_est = np.mean(rs_normal) if rs_normal else None
    return r_est
...
def determine_dataarray_chunks(
    ...
    chunk_dims: Optional[Sequence[str]] = None,
